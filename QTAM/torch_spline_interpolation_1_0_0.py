...
class SplineInterpolate2D(nn.Module):
    def __init__(self, num_t_bins, num_f_bins, kx=3, ky=3, sx=0.0, sy=0.0, logf=False, frange=(8, 500), device='cpu'):
        super().__init__()
    ...
    def inverse_to_samples(self, x, y, C, tx, ty, kx, ky):
        """
        Recover the original sample values Z from spline coefficients.
    
        Args:
            x: 1D tensor of original x sample locations
            y: 1D tensor of original y sample locations
            C: spline coefficient tensor (B, C, mx, my)
            tx, ty: knot vectors
            kx, ky: spline degrees
    
        Returns:
            Z_recovered: tensor with same shape as original Z input
        """
        # Just evaluate at the original nodes
        Z_recovered = self.evaluate_bivariate_spline_torch(x, y, C, tx, ty, kx, ky)
        return Z_recovered
