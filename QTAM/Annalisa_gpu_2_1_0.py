...
class STFTWhiten(nn.Module):
    def __init__(self, 
                 ...
                 duration: float, #duration of the time series in seconds
                 psd_nfft: int = 2048, 
...
#        truncation_window_size = int(np.round(1/5*self.duration*(1+2*border_fraction))) #the value 6/5 was found empirically by doing many trials of whitenings
        truncation_window_size=int(np.round(0.2*duration)) # the value 0.2 was found empirically by doing many trials
        if truncation_window_size==0:
    ...
    def _border_mitigation(self, x):
        ...
        #taper_amount=int(*self.sample_rate)
        taper_amount=int(0.5*x.shape[-1])
...
#        if self.duration<=1/3:
#            raise ValueError("The time series must have a minimum length of 1/3 seconds.")
        if 2*self.duration*(1+2*self.border_fraction)<1:
            minimum_border_fraction=1/4/self.duration-1/2.
            raise ValueError("The border fraction is too small for these parameters, choose a border fraction of at least "+str(minimum_border_fraction))
        ...
        x_whitened = x_whitened / torch.sqrt(torch.tensor(self.sample_rate))
