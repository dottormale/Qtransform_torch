...
class SingleQTransform(torch.nn.Module):
    ...
    def interpolate(self, num_f_bins, num_t_bins, device):
        ...
        xout= torch.linspace(0.0,0.0+self.duration, steps=num_t_bins)
        
        # Interpolate along the time dimension using natural cubic spline

        #define NN for 1d interpolation

        spline_interpolate_1d=SplineInterpolate1D(num_t_bins).to(device)
        
        #group qtiles by length
        lengths = [tensor.shape[-1] for tensor in self.qtiles]  
        if self.qtiles[0].shape[0]==1:
            self.qtiles=[tensor.transpose(0,1) for tensor in self.qtiles]
        ...
        unique_lengths = list(length_counts.keys()) # Extract unique lengths
        
        Z_list=[]
        for j,length in enumerate(unique_lengths): 
    
            # Collect tensors of the same length
            same_length_tensors = [tensor for tensor in self.qtiles if tensor.shape[-1] == length]
            
            
            group_tensor = torch.stack(same_length_tensors)  # [R_i, B, C, T_i]
            Z_list.append(group_tensor)
        ...
        e1d=time.time()
        #print(f'1d interpolation took: {e1d-s1d} s')
        
        time_interp_tiles=torch.cat(time_interp_tiles,dim=0)
        ...
        if self.logf:
            ...
            #yout = torch.arange(self.frange[0], self.frange[1],(self.frange[1] - self.frange[0])/num_f_bins)
        ...
        resampled=spline_interpolate_2d(time_interp_tiles.permute(1,2,3,0),xin=xout,xout=xout,yin=self.freqs,yout=yout)    


        # Interpolate phase if required
        if self.phase_mode:
            # 1D interpolation for phase
            if self.phase_qtiles[0].shape[0]==1:
                self.phase_qtiles=[tensor.transpose(0,1) for tensor in self.phase_qtiles]
            Z_phase_list=[]
            for j,length in enumerate(unique_lengths): 

                # Collect tensors of the same length
                same_length_phases = [tensor for tensor in self.phase_qtiles if tensor.shape[-1] == length]


                group_tensor = torch.stack(same_length_phases)  # [R_i, B, C, T_i]
                Z_phase_list.append(group_tensor)
            ...
            phase_interp = spline_interpolate_2d(
                time_interp_phase_tiles.permute(1,2,3,0), xin=xout, xout=xout,
